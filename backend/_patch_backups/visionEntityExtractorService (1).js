...
function cleanVietnamAddressCandidate(value) {
  let text = cleanVietnamAddressOcrText(value)
    ...
    .replace(/\b(?:ĐT|DT|SĐT|SDT|PHONE|HOTLINE)\b.*$/gi, ' ')
...
function recoverEmbeddedFrameAddressCandidates(value) {
  ...
  const results = []
  const seen = new Set()
  const pattern = /\b\d{1,5}\/[A-Za-z0-9]{1,8}\s+[A-Za-zÀ-ỹ][A-Za-zÀ-ỹ'.-]*(?:\s+[A-Za-zÀ-ỹ][A-Za-zÀ-ỹ'.-]*){1,7}\b/gu
  for (const match of compacted.matchAll(pattern)) {
    ...
    if (hasAddressMenuNoiseText(candidate) || isPromotionalOrCountText(candidate)) continue
    seen.add(key)
...
function frameAddressCandidates(normalizedEvidence = {}) {
  ...
  const recoveredCandidates = recoverableFrameSources.flatMap((item) => {
    ...
    return recoverEmbeddedFrameAddressCandidates(rawText).map((address) => ({
